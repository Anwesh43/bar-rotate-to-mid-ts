...
class BRTMNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BRTMNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBRTMNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BRTMNode {
        var curr : BRTMNode = this.prev  
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BarRotateToMid {

    curr : BRTMNode = new BRTMNode(0)
    dir : number = 1
